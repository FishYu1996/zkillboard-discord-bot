...
def statistics(damages, language, name):
    ...
    for weapon, stats in weapon_stats["to"].items():
        ...
        efficiency_percentages = {
            eff: (count / total_efficiency_occurrences * 100)
            for eff, count in efficiency_count.items()
        }
...
def draw_plots_from_stats(stats, language):
    ...
    if stats["rep_receive"] or stats["damage_receive"]:
        rep_dmg_receive_img = rep_dmg_receive_plot(
            stats["rep_receive"],
            [(item[0], item[1]) for item in stats["damage_receive"]],
            language,
        )
    ...
    if total_rep > 2000:
        ...
        if stats["damage_done"]:
            ...
            main_accr = (
                main_eff.get("Hits", 0)
                + main_eff.get("Penetrates", 0)
                + main_eff.get("Smashes", 0)
                + main_eff.get("Wrecks", 0)
                + main_eff.get("命中", 0)
                + main_eff.get("穿透", 0)
                + main_eff.get("强力一击", 0)
                + main_eff.get("致命一击", 0)
...
def rep_dmg_receive_plot(list1, list2=[], language="en"):
    ...
    rects2 = ax.bar(x, data2, width, bottom=data1, label="DMG received", color="red")
...
def damage_done_plot(list1, language="en"):
    ...
    if language == "zh":
        names1 = ["Misses", "轻轻擦过", "擦过", "命中", "穿透", "强力一击", "致命一击"]
